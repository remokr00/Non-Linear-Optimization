...
# Initialization of variables for a truss structure composed of 36 bars
...
nodes_initial = np.array([
    [0, 2 * L], [L, 2 * L], [2 * L, 2 * L], [3 * L, 2 * L], [4 * L, 2 * L],
    [0, L], [L, L], [2 * L, L], [3 * L, L], [4 * L, L],
...
def truss_objective_function_area(areas):
    ...
    penalty_total_3 = (np.sum(stress_norm))
...
def truss_objective_function_coord(coordinates):
    ...
    # if two or more nodes have at least one same coordinate or are too close to each other, add a large penalty
...
def calculate_axial_forces(displacements, bars, nodes, areas, E):
    ...
    for (i, j), area in zip(bars, areas):
        ...
        c = np.cos(theta)
        s = np.sin(theta)
        k_local = local_stiffness_matrix(E, area, L, theta)
        index_i = [2 * i, 2 * i + 1]
        index_j = [2 * j, 2 * j + 1]
...
def plot_truss_structure(nodes, figure_number):
    ...
    for index, (x, y) in enumerate(nodes):
        if index in [0, 5, 10, 14]:  # Nodi fissi
...
for index, (x, y) in enumerate(nodes_initial):
    if index in [0, 5, 10, 14]:  # Assuming nodes 0, 5, 10 are fixed
...
# Add nodes with different colors for fixed and free nodes
for index, (x, y) in enumerate(nodes_initial):
    if index in [0, 5, 10]:  # Assuming nodes 0, 5, 10 are fixed
        ax.plot(x, y, 'ro', markersize=10, marker='>')  # Red color for fixed nodes
    else:
        ax.plot(x, y, 'go', markersize=5)  # Green color for other nodes
...
# Create plots for each set of optimized coordinates
