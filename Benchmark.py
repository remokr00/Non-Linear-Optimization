...
displacements = np.full(2 * n_nodes, 1e-2)
fixed_nodes_area = [0, 1, 6, 7]
fixed_nodes_size = [0, 1, 6, 7, 10, 11]
...
external_forces[9] = -force_magnitude  # Forza applicata in y per il nodo 5
external_forces[11] = -force_magnitude  # Forza applicata in y per il nodo 6
# Initial coordinates of nodes in a 2D space
...
# Connectivity of each bar: tuples (start_node, end_node)
bars = [
    (0, 1), (1, 2),
    (3, 4), (4, 5),
    (1, 4), (2, 5),
...
def truss_objective_function_area(areas):
    global rho, L, nodes_initial, bars, E, external_forces, sigma_max, fixed_nodes_area

    K_global = assemble_global_stiffness(nodes_initial, bars, E, areas, fixed_nodes_area)
    try:
    ...
    penalty_total_3 = (np.sum(stress_norm)) # * 1e-2
...
def truss_objective_function_coord(coordinates):
    global rho, L, area_constant, bars, E, external_forces, sigma_max, displacements, fixed_nodes_size
    ...
    K_global = assemble_global_stiffness(coord_for_k, bars, E, areas, fixed_nodes_size)
    try:
        displacements = equilibrium_constraint(K_global, external_forces)
        max_displacement = np.max(np.abs(displacements))  # Calculate maximum displacement for normalization
        displacements_norm = displacements / max_displacement  # Normalize displacements
    ...
    lengths_of_bars = compute_lengths(coord_for_k, bars)
    ...
    penalty_total = np.sum(non_negative_length_constraints_norm) + np.sum(displacement_penalty) + (np.sum(stress_norm)) + np.sum(coord_constraints)
    ...
    for i in range(0, len(coord_for_k)):
        if coord_for_k[i][1] < 0 or coord_for_k[i][1] > L:
...
def assemble_global_stiffness(nodes, bars, E, areas, fixed_indices):
    ...
    for fi in fixed_indices:
        ...
        K_global[fi, fi] = 1e10  # Consider making this a large finite value instead of 1e10

    return K_global

def equilibrium_constraint(K_global, external_forces):
    if np.linalg.det(K_global) == 0:
        raise ValueError("Singular matrix detected in equilibrium constraint.")
...
def plot_truss_structure(nodes, figure_number):
    ...
    ax.set_title(f'Optimized Truss Structure')
    plt.show()
...
for index, (x, y) in enumerate(nodes_initial):
    if index in [0, 3]:  # Assuming nodes 0 and 3 are fixed
...
def truss_draw():
    ...
    # Add nodes with different colors for fixed and free nodes
    for index, (x, y) in enumerate(nodes_initial):
        if index in [0, 3, 5]:  # Assuming nodes 0 and 3 are fixed
            ax.plot(x, y, 'ro', markersize=10, marker='>')  # Red color for fixed nodes
        else:
            ax.plot(x, y, 'go', markersize=5)  # Green color for other nodes
...
fixed_nodes = [1, 4, 6]
fixed_values = [0, L, 0, 0, 2 * L, 0]
bounds = [(0, 5 * L)] * 12
max_iteration_2 = 100
a = 0.0001
[ris_1, eval_1, iter_ris_1, optimum_1] = boa(truss_objective_function_coord, bounds, num_butterflies=50, max_iter=max_iteration_2, a=a, fixed_nodes=fixed_nodes, fixed_values=fixed_values)
[ris_2, eval_2, iter_ris_2, optimum_2] = boa(truss_objective_function_coord, bounds, num_butterflies=100, max_iter=max_iteration_2, a=a, fixed_nodes=fixed_nodes, fixed_values=fixed_values)
[ris_3, eval_3, iter_ris_3, optimum_3] = boa(truss_objective_function_coord, bounds, num_butterflies=150, max_iter=max_iteration_2, a=a, fixed_nodes=fixed_nodes, fixed_values=fixed_values)
