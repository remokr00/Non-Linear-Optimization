...
lengths_opt = compute_lengths(nodes_initial_opt, bars_opt)
external_forces_opt = np.zeros(2 * n_nodes_opt)  # External forces assumed to be zero initially
external_forces_opt[18] = force_magnitude  # Forza applicata in x per il nodo 10
external_forces_opt[28] = force_magnitude  # Forza applicata in x per il nodo 15
external_forces_opt[29] = -force_magnitude  # Forza applicata in y per il nodo 15
...
def truss_objective_function_coord(coordinates):
    ...
    K_global = assemble_global_stiffness(coord_for_k, bars, E, areas, [0,1,6,7,10,11])
    try:
        displacements = equilibrium_constraint(K_global, external_forces)
        max_displacement = np.max(np.abs(displacements))  # Calculate maximum displacement for normalization
        displacements_norm = displacements / max_displacement  # Normalize displacements
